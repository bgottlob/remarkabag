defmodule Remarkabag.Entry do
  ...
  def by_url_hash(url_hash) do
    ...
    for record <- records, do: from_record(record)
  end
  ...
  def by_not_synced() do
    {:atomic, records} = Mnesia.transaction(fn ->
      Mnesia.index_read(__MODULE__, false, :synced)
    end)
  ...
  def need_to_remarkable_sync() do
    ...
    for record <- records, do: List.to_tuple(record)
  end
  ...
  def by_downloaded_path_and_synced(downloaded_path, synced) do
    downloaded_path_guard = case downloaded_path do
      true -> {:is_binary, :"$2"}
      false -> {:==, :"$2", nil}
    end
  ...
  def write_from_wallabag(wallabag_id, mimetype, title, url, url_hash) do
    ...
    # TODO Do I really need to return an Entry here?
    entry
  end
